...
def render_system_status() -> None:
    ...
    data_updated_text = latest_data_mtime.strftime("%Y-%m-%d %H:%M:%S") if latest_data_mtime else "无"

    latest_run = _latest_run() or "无"
    ...
    c1.metric("数据更新时间", data_updated_text)
    c2.metric("最新运行", latest_run)
    c3.metric("数据完整性", integrity)
    c4.metric("系统健康", system_status)
...
def render_portfolio_snapshot(run_id: str) -> None:
    ...
    row1 = st.columns(4)
    row1[0].metric("总收益", _fmt_pct(total_return))
    row1[1].metric("年化收益", _fmt_pct(cagr))
    row1[2].metric("夏普比率", _fmt_num(sharpe, 3))
    row1[3].metric("卡玛比率", _fmt_num(calmar, 3))

    row2 = st.columns(4)
    row2[0].metric("最大回撤", _fmt_pct(max_dd))
    row2[1].metric("年化波动", _fmt_pct(ann_vol))
    row2[2].metric("回撤持续天数", _fmt_num(dd_duration_days, 0))
    row2[3].metric("当前回撤", _fmt_pct(current_dd))

    if not plot_df.empty:
        st.line_chart(plot_df.set_index("ts")["nav"])
    else:
        st.info(f"{run_id}: 净值曲线缺失，已跳过图表")
...
def render_recent_runs_table(runs_df: pd.DataFrame) -> None:
    ...
    selected_run_id = st.selectbox("运行ID", options=options, index=options.index(default_run_id), key="recent_runs_selector")
    st.session_state["selected_run_id"] = selected_run_id

    c1, c2 = st.columns(2)
    with c1:
        if st.button("打开详情", use_container_width=True, key="recent_open_detail"):
            st.query_params["run_id"] = selected_run_id
            st.switch_page("pages/2_run_detail.py")
    with c2:
        if st.button("加入对比", use_container_width=True, key="recent_add_compare"):
            ...
            if selected_run_id not in basket:
                ...
                st.success(f"已加入对比：{selected_run_id}")
            else:
                st.info(f"已在对比篮子中：{selected_run_id}")
...
def render_data_health() -> None:
    ...
    def _scan_dir(root: Path) -> dict[str, object]:
        ...
        if file_count == 0:
            ...
            status = "正常"
            last_updated = datetime.fromtimestamp(latest_mtime).strftime("%Y-%m-%d %H:%M:%S") if latest_mtime else "无"
...
def render_quick_actions() -> None:
    ...
    with c1:
        if st.button("查看运行", use_container_width=True, key="qa_view_runs"):
            st.switch_page("pages/1_runs.py")
    with c2:
        if st.button("对比运行", use_container_width=True, key="qa_compare_runs"):
            st.switch_page("pages/3_compare_runs.py")
    with c3:
        force_update_all = bool(st.session_state.get("force_update_all", False))
        if st.button("更新全部", use_container_width=True, key="qa_update_all"):
            _run_update_stream_ui(force_update_all, sidebar=False)

    st.caption(f"强制全量更新（侧边栏）：{'开启' if st.session_state.get('force_update_all', False) else '关闭'}")
